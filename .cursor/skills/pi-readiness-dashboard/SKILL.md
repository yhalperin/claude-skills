...
Produces one self-contained HTML file - no server, no build step - visualizing Jira Theme issues for a chosen Program Increment (PI). Layout, top to bottom: 5 KPI cards (Readiness Index, **Net New Scope**, Carry-Over, Ready/Planned, Not Ready), a full-width "AI & Strategic Insights" band, a status/division bar chart + portfolio composition panel (**two** side-by-side donuts, each broken down by real raw Jira status only, canonically ordered: **Net Scope** (671) and the full **Total**, incl. In Progress (840 = 671 + 169) - not a third "category" bucket - plus a single combined breakdown list below them, one row per raw status tagged Net Scope/Carry-Over, never two near-duplicate lists repeating the same status+count), an **Ownership Responsibility Matrix** row (Product Management vs R&D counters + split donut, Net New Scope only), and a full-width themes registry table. Every status-related display (badges, chart legends, breakdown chips) shows the exact raw Jira status (e.g. `Open`, `HL Product Discovery`, `HL Dev Discovery`, `In Progress`) rather than a collapsed label, using a status-accurate color scheme: green = Ready, teal = Planned, **blue = In Progress**, slate = Open, amber = HL Product Discovery, violet = HL Dev Discovery. There is no "Quality Flags" KPI card - owner/flag risk is only surfaced in the table's Flags/Risk column and the AI insights band. The visual design is already built into `assets/template.html` - do not redesign it per run, only feed it fresh data.
...
The header prominently shows an amber **"Jira Data as of \<date\>"** badge (plus a matching line in the footer note under AI & Strategic Insights) - this is deliberately NOT styled or worded like a live/real-time indicator, because it isn't one: it's whatever date was passed to `render_dashboard.py --fetched-at` at generation time, i.e. when the embedded Jira data was actually pulled, which can be well before "today" if the dashboard hasn't been refreshed since. A separate, de-emphasized "Dashboard last generated \<timestamp\>" note nearby is just when this HTML file itself was produced (which can differ - e.g. a template tweak re-rendered the same old data days later) - never conflate the two, and always get `--fetched-at` right (see step 7 below).
...
The **PI Readiness Index** itself (the headline "Net Readiness" score) is `(ready + planned) / (total - carryOver) * 100` - see [DATA_SCHEMA.md](DATA_SCHEMA.md#pi-readiness-index-formula) for the full rationale. Carry-Over is excluded from both sides of that ratio (already rolled over from a prior PI); Not Ready backlog stays in the denominator only, so it still dilutes the score. The KPI card also always shows a "bottom line" pair of numbers - the headline **Net Readiness** repeated, alongside a second, distinct **of Total** coverage metric where "In Progress" flips to counting *toward* readiness: `(ready + planned + carryOver) / total * 100` - see [DATA_SCHEMA.md](DATA_SCHEMA.md#two-readiness-lenses-different-numerator-and-denominator---not-the-same-ratio-at-two-scales) for why the two use different numerators, not just different denominators.
...
7. **Render.** Write the exact JQL from step 2 to a small text file first (e.g. `jql.txt`) and pass it via `--jql-file` - on Windows/PowerShell, passing the JQL directly via `--jql` is unreliable because the embedded double quotes get mangled by shell/CRT argument parsing. **Always pass `--fetched-at`** with the actual date/time you ran the Jira MCP queries in step 2 (e.g. `"2026-07-14 17:16"` or just `"2026-07-14"`) - this is what the dashboard's header badge and footer show as "Jira Data as of", and it is deliberately a SEPARATE, explicit value from whenever this render script happens to run (which could be days later, e.g. after a template tweak). Never let a re-render make old Jira data look freshly fetched.
   ```bash
   python scripts/render_dashboard.py --themes themes.json --pi "27-Q1" --division "All" --jql-file jql.txt --fetched-at "2026-07-14 17:16"
   ```
   Pass the exact `--division` value from step 1 (e.g. `"Secrets Manager"`) when scoped, matching a key in `hr_tree.json`. Add `--out <path>` to control the filename, or `--no-open` to skip auto-opening. This writes a stable filename derived from PI + division (e.g. `pi_readiness_27-q1_all.html`) next to the themes file. `--timeline` defaults to the bundled `assets/pi_timeline.json`; only pass it explicitly if using a different timeline export. If `--fetched-at` is omitted, it falls back to `--themes`' file-modified time with a printed warning - don't rely on this fallback when you know the real fetch time.

8. **Report a fetch summary**: PI, Division scope, total Themes fetched, per-raw-status counts, whether a PI Timeline was found/rendered (and today's highlighted phase), the Jira-data-as-of date passed via `--fetched-at`, and the output path. Mention that Division, Group, and Throughput Type can all be further drilled into live in the browser (top-right dropdowns) without re-running anything - that filtering works against whatever Themes were embedded at render time. Also mention the dashboard defaults to "A. Feature + Empty" Throughput Type, so the KPI cards on first load are narrower than the total fetched count.
