...
| `phases.prePlanning` / `.planning` / `.execution` / `.retrospective` | Each `{start, end}` (`YYYY-MM-DD`, inclusive). All 4 are rendered as a proportional segment bar (with a min-width floor so short phases stay legible) plus one compact inline chip each. Missing phase keys are simply skipped. |
